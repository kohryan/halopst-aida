...
const systemPrompt = {
  ...
  jika ada yang bertanya tentang analisis data, berikan rekomendasi analisis data yang sesuai dan contoh kode R atau Python dari analisis sederhana sampai analisis yang kompleks. Jika ada judul tahapannya, gunakan format paragraf saja. 
  `
};
...
function formatToHtml(text) {
  ...
  // Convert Python code blocks (```python ... ```) to <pre><code>
  formattedText = formattedText.replace(/```python([\s\S]*?)```/g, function(match, p1) {
    // Add a class to format heading inside code blocks differently
    const codeContent = p1
      .trim()
      // Convert any headers inside Python code blocks to smaller size
      .replace(/^# (.*)$/gim, '<span class="code-heading">$1</span>')
      .replace(/^## (.*)$/gim, '<span class="code-subheading">$1</span>');

    return '<pre><code class="language-python">' + codeContent + '</code></pre>';
  });

  // Convert Python code blocks (```python ... ```) to <pre><code>
  formattedText = formattedText.replace(/```r([\s\S]*?)```/g, function(match, p1) {
    // Add a class to format heading inside code blocks differently
    const codeContent = p1
      .trim()
      // Convert any headers inside Python code blocks to smaller size
